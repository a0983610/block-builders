...
const ENG = (function () {
  ...
  /* 火／火星粒子。240 是「一次爆炸的火球 + 幾棟在燒」的量；
     ...
     v1.200 再到 1280：爆炸的火星改成「多而細」，一發最多吃到規則那邊的 HOT_BURST
     （560 顆），跟一場齊射同台時 960 會把後面那幾百條拖線默默切掉。
  ...
  /* 藍色閃電的線段上限。一道電折六段、可能再帶一條分岔，一處爆點同時最多六道 → 不到 60；
     ...
     **×5（v1.165）**：打雷改成一次點擊三朵雲（見 game-tools 的 STORM_TRIO），
     三次點擊就是九朵，各自最密同時兩道 × 19 段 ＝ 342 段，加上爆裂魔法的 126
     要 468——停在 288 的話最後那朵的電會被默默切掉半截。 */
  const MAXBOLT = 96 * 5;
  ...
  /* 兵器：一把最多 WEAP_PARTS 塊方塊，全部塞進同一顆 InstancedMesh（跟核彈同一個做法）。
     ...
     **清單尾巴**——putWeapons 直接切掉，畫面上會有一批箭憑空消失。
     v1.175 再加到 1000：箭雨改成 80 人射**五**輪（一隊 400 支，規則那邊的 AR_KEEP
     就是這個數），560 ＋ 400 ＝ 960。多的 160 個 instance 只是一段 buffer，
     沒東西可畫的時候 count 就是 0（e2e〈容量留得夠〉照常數守著這條算式）。 */
  const WEAP_MAX = 1000, WEAP_PARTS = 8;
  let weapMesh = null;
  ...
  /* 造型座標上的幾個高度／寬度。**規則那邊直接讀這幾個**算刃掃到哪（同 DOZ_W 的用意）：
     ...
     ② 「劍尖往下一小段才是攻擊點」——判定與瞄準用 HIT，不是刃尖：刃尖那一段照樣
        畫得出來、但不砍；瞄的那兩點落在 HIT 上。v1.169 第一版取 TIP − 0.060
        （5.28 單位），使用者看過說「再往下一點」，所以乘二成 TIP − 0.120
        （刃長 0.704 的 17.0%）。**v1.173 再往下挪到刃的正中央**（使用者：「大劍打擊點
        改到劍刃中央」）＝ TIP − 0.352 ＝ 刃長的一半，見下面那一行。 */
  const SWORD_PIVOT = -0.620, SWORD_EDGE = -0.190, SWORD_TIP = 0.514,
        /* v1.173：攻擊點挪到**刃的正中央**（使用者：「大劍打擊點改到劍刃中央」）。
  ...
  /* 造型：騎士巨劍（雙刃直刃、中脊開槽、平直十字護手、皮革纏柄圓劍首）。
     ...
     開發筆記〈大劍〉：刃尖收四階（收兩階像螺絲頭）、每一階跟上一階重疊（不然接縫開）、
     刃的寬厚比拉到 5:1 以上（3:1 看起來是根鐵條不是刃）。
     護手四款也是看圖挑的，使用者選「平直十字」——原本兩端往刃側上揚 0.45 弧度，
     翼根又擺在橫樑外面留了一條縫，看起來像貼上去往上翹的耳朵。
     v1.162 使用者要「長度約兩倍、比例也微調」，看到之後再回一句「目前的大劍太粗了」：
     世界長度那一邊直接乘二（見 SW_LEN_*），這裡把比例往「更長更細」挪——
  ...
  const SWORD_PART = [
    ...
    /* 柄 */
    { p: [0, -0.320, 0], s: [0.036, 0.190, 0.036], c: SW_GRIP },
    { p: [0, -0.262, 0], s: [0.043, 0.015, 0.043], c: SW_WRAP },
    { p: [0, -0.320, 0], s: [0.043, 0.015, 0.043], c: SW_WRAP },
    { p: [0, -0.378, 0], s: [0.043, 0.015, 0.043], c: SW_WRAP },
  ...
  /* 積木池上限。**地標 ＋ 小人的村子兩份都要塞得下**，因為它們共用這一個池子：
       ...
       · 村子裡的樹（v1.153，同一份 homes）：棵數不靠「一人一棵」擋，直接用塊數擋，
         上限就是 game-workers.js 的 TREE_BUDGET ＝ 1800。
     ...
     v1.153 村子多了樹，同一條算式再加 TREE_BUDGET：9932 ＋ 5405 ＋ 1800 ＝ 17137，
     餘裕照舊：18000。多出來的 2000 格是 2000×(16+3) 個 float ≈ 152KB，一次性配置。
       · 城牆（v1.186，同一份 homes）：整圈的塊數是幾何算出來的，不靠預算擋——
         實測最大的那一座（金門大橋 9000 建材、牆半徑 94）是 **11396 塊**
         （v1.194 四面各開一座門樓之前是 10616）。牆是 3 格厚的空心牆，
         見 開發筆記〈事件二：城牆〉〈四面都開門，門樓改成歐式的尖拱門〉。
     所以再加一份：9932 ＋ 5405 ＋ 1800 ＋ 11396 ＝ 28533 → **29000**。
     成本：instanceMatrix 是 16 個 float ×29000 ≈ 1.9MB，一次性配置，不影響每幀；
     每幀成本看的是**實際有幾塊**，不是這個上限（量到的數字見 開發筆記〈積木池上限〉）。 */
  const MAXB = 29000;
  ...
  /* ── 飛石與隕石的球（v1.193）──
     使用者：「投出去的石頭改成球型」「隕石也順便改成球型」。兩邊以前都掛在 unit
     那顆方塊上，所以天上飛的是一個轉來轉去的木箱。
     用 IcosahedronGeometry 的 detail 1（80 面）而不是經緯球：看得出刻面、像一顆
     粗鑿的石球，跟整支程式的方塊風格對得起來，三角形也只有同精細度經緯球的一半。
     **半徑給 0.5 不是 1**：呼叫端的 s 一路都是「這顆多大」（＝原本那個方塊的邊長，
     也是 sweepRock 拿 s×0.5 當碰撞半徑的那個 s），半徑 1 的球乘上去會整整大一倍，
     而且畫出來的跟判定用的就對不起來了。兩處共用同一顆幾何，所以只做一次。 */
  let _rockGeo = null;
  function rockGeo() { return _rockGeo || (_rockGeo = new T.IcosahedronGeometry(0.5, 1)); }
  /* 球**不能**用 voxelMaterial：那一份的深色邊是照「單位方塊的 local 座標」算的
     （0.5 −「三軸絕對值的第二大者」），套在球上整顆都判成面中央，一點邊都畫不出來
     ——白付一次 shader 注入。改用跟保齡球同一路的 Lambert，加 flatShading 讓每個刻面
     各吃各的光：three 的多面體幾何預設是球面法線（平滑），不開這個旗標看起來就是
     一顆光滑的蛋，轉起來也看不出它在轉。
     見 開發筆記〈整台照參考圖重做、甩臂方向掉頭、石頭改成球〉第五節 */
  function rockMaterial(color) {
    return new T.MeshLambertMaterial({ color, flatShading: true });
  }
  ...
  function init(cvs) {
    ...
    /* 破壞道具：鐵球。v1.116 起可以同時有好幾顆（使用者：「保齡球可以多顆」），
       ...
       沒球的時候 visible = false，一個 call 都不吃（見 開發筆記〈效能〉）。 */
    ...
    rockMesh = new T.InstancedMesh(rockGeo(), rockMaterial(0x6b6660), MAXROCK);
    ...
    /* 隕石：一顆一個 instance。石頭本體走 instance color，越接近落地燒得越紅
       ...
       幾何跟投石機的飛石同一顆球（v1.193，使用者：「隕石也順便改成球型」）。 */
    meteorMesh = new T.InstancedMesh(rockGeo(), rockMaterial(0xffffff), MAXMET);
  ...
  /* 投石機（v1.193 照使用者給的參考圖整台重做）。t：{x, z, a 面向, arm 拋臂角度, sa 索的角度}
     ...
     造型表的欄位：
       p    位置（機台座標，**+z 朝目標**）  s 尺寸  c 顏色
       r    固定旋轉 [x,y,z]——斜撐與輪輻要用（以前的表只有「跟著臂轉」一種旋轉）
       d    掛在拋臂上、離支點多遠（正＝投石端＝**朝 −z**、負＝配重端＝朝 +z）
       hang 掛點再往下吊多少（配重是吊著的，所以不跟著臂的升降走）
       spin 跟著臂一起轉（臂身、臂上的鐵箍）；沒給就維持直立
       sd   掛在投石索上、離臂尖多遠（索有自己的角度，見 trebSling）
     有 d／sd 的那幾塊只吃 p[0]（左右偏移），y／z 是每一幀算出來的。

     **投石端朝 −z、配重端朝 +z**——也就是配重掛在**朝目標**那一側。v1.193 第一版
  ...
  const TREB_SLING = 2.1;           // 投石索：從臂尖再延出去多長，石頭從它末端離手
  ...
  const TR_ARMC = 0x5f4126, TR_ROPE = 0xc9b083, TR_CWC = 0x5a5048;

  /* 一個輪子＝兩塊交叉的板（0° 與 45°）。單一方塊只會像個木箱，交叉之後是個八角星
     ——輪輻的樣子。尖角剛好落在輪緣（半徑 TREB_WHEEL），所以最低那個尖點正好觸地。 */
  function trebWheel(x, z) {
    const w = TREB_WHEEL * Math.SQRT2;
    return [
      { p: [x, TREB_WHEEL, z], s: [0.36, w, w], c: TR_WOOD },
      { p: [x, TREB_WHEEL, z], s: [0.36, w, w], c: TR_WOOD, r: [Math.PI / 4, 0, 0] }
    ];
  }
  /* 一根斜撐：給「從哪撐到哪」（同一個 x 平面上的兩點），長度與傾角自己算
     ——支點高度或底盤長度一改就跟著對，不必回頭改死數字
     （見 開發筆記〈不要寫死會隨改動變動的數字〉）。 */
  function trebLeg(x, z0, y0, z1, y1, w, c) {
    const dy = y1 - y0, dz = z1 - z0;
    return { p: [x, (y0 + y1) / 2, (z0 + z1) / 2],
             s: [w, Math.hypot(dy, dz), w], c, r: [Math.atan2(dz, dy), 0, 0] };
  }
  const TREB_PART = [].concat(
    [
      /* 底盤：兩根縱樑、前後橫樑，前橫樑上一道藍鐵箍。
         前橫樑擺在 2.72 不是 2.45：配重盪到低處時滾筒最前緣會走到 z 2.29，
         擺 2.45 的話那兩塊會插在一起（實測掃過整個擺程量出來的）。 */
      { p: [-1.45, TREB_BED, 0],    s: [0.66, 0.55, 6.0],  c: TR_BEAM },
      { p: [1.45, TREB_BED, 0],     s: [0.66, 0.55, 6.0],  c: TR_BEAM },
      { p: [0, TREB_BED, 2.72],     s: [3.5, 0.5, 0.56],   c: TR_WOOD },
      { p: [0, TREB_BED, -2.62],    s: [3.5, 0.5, 0.66],   c: TR_WOOD },
    ...
    trebWheel(-1.85, -1.95), trebWheel(1.85, -1.95),
    [
      /* A 形立架：每側一前一後兩根斜腿撐到支點，中間一根橫拉桿 */
      trebLeg(-1.32, 2.15, TREB_BED + 0.3, 0, TREB_PIV, 0.5, TR_WOOD),
      trebLeg(1.32, 2.15, TREB_BED + 0.3, 0, TREB_PIV, 0.5, TR_WOOD),
      trebLeg(-1.32, -2.15, TREB_BED + 0.3, 0, TREB_PIV, 0.5, TR_WOOD),
      trebLeg(1.32, -2.15, TREB_BED + 0.3, 0, TREB_PIV, 0.5, TR_WOOD),
      { p: [-1.32, 3.2, 0],     s: [0.34, 0.34, 2.1],  c: TR_WOOD },
      { p: [1.32, 3.2, 0],      s: [0.34, 0.34, 2.1],  c: TR_WOOD },
      { p: [0, TREB_PIV, 0],    s: [3.5, 0.42, 0.42],  c: TR_IRON },   // 支點軸
      /* 側邊那一層站人的平台（參考圖右上那一層） */
      { p: [2.45, 2.75, -0.2],  s: [1.5, 0.3, 3.2],    c: TR_DECK },
      { p: [2.95, 1.9, -0.2],   s: [0.3, 1.4, 0.3],    c: TR_BEAM },   // 平台撐柱
      { p: [3.05, 3.4, 1.2],    s: [0.24, 1.0, 0.24],  c: TR_WOOD },   // 欄杆柱
      { p: [3.05, 3.4, -1.6],   s: [0.24, 1.0, 0.24],  c: TR_WOOD },
      { p: [3.05, 3.83, -0.2],  s: [0.24, 0.22, 3.2],  c: TR_WOOD },   // 欄杆橫桿
      /* 拋臂那一組。臂身從配重掛點一路長到臂尖，所以中心偏在投石端這一側 */
      { d: (TREB_ARM - TREB_CW) / 2, s: [0.46, 0.46, TREB_ARM + TREB_CW],
        c: TR_ARMC, spin: 1 },
      { d: TREB_ARM - 1.0, s: [0.56, 0.56, 0.3], c: TR_BLUE, spin: 1 },
      { d: -TREB_CW, hang: 0.45, s: [0.22, 0.8, 0.22], c: TR_IRON },   // 配重吊桿
      { d: -TREB_CW, hang: 1.05, s: [1.6, 1.4, 1.35],  c: TR_CWC },    // 配重滾筒
  ...
  /* 臂上離支點 d 的那一點在機台座標的位置。臂的方向（從支點指向投石端）是
     (z, y) = (−cos arm, −sin arm)：arm 0 ＝ 長臂水平朝**後**，正角＝長臂壓低在後
     （配重高舉在前），負角＝長臂甩到後上方。 */
  function trebAt(arm, d) {
  ...
  function putTrebs(list) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      for (let k = 0; k < TREB_PARTS; k++) {
        ...
        if (b.sd !== undefined) {
          /* 掛在投石索上：從臂尖沿著索自己的角度往外算。
             盒子的 local +z 繞 X 轉 ρ 之後指到 (cos ρ, −sin ρ)，要它指到
             (cos sa, sin sa)，所以 ρ ＝ −sa。 */
          scratchB.position.set(bx, tip.y + Math.sin(sa) * b.sd,
                                tip.z + Math.cos(sa) * b.sd);
          scratchB.rotation.set(-sa, 0, 0);
        } else if (b.d !== undefined) {
          /* 掛在拋臂上（見 trebAt）。hang 是再往下吊多少——配重是吊著的，
             跟著臂的**位置**走、不跟著臂的角度轉。
             臂身要指到 (−cos arm, −sin arm)，同上一段解出來 ρ ＝ π − arm。 */
          scratchB.position.set(bx, TREB_PIV - Math.sin(t.arm) * b.d - (b.hang || 0),
                                -Math.cos(t.arm) * b.d);
          scratchB.rotation.set(b.spin ? Math.PI - t.arm : 0, 0, 0);
        } else {
          scratchB.position.set(bx, b.p[1], b.p[2]);
          if (b.r) scratchB.rotation.set(b.r[0], b.r[1], b.r[2]);
  ...
  /* 消防車。m：{x, z, a 朝向, bob 引擎抖動, bk 警示燈亮不亮}
     車頭朝 local +Z，跟推土機、投石機同一套擺位。**水砲固定朝車尾**（v1.187，使用者：
     「改成用車尾噴水」）——車子停下來會把**車尾**轉向火場，所以砲口自然對著要噴的地方，
     不必再單獨轉砲塔。砲管末端跟車尾切齊（z −3.63），所以它一格都沒有把車變長：
     規則那邊的車體占地是從這張表算出來的（見 game-tools 的 FT_BODY 與 FT_NOZ）。 */
  const TRK_PART = [
    ...
    { p: [0, 2.9, -2.33], s: [0.34, 0.34, 2.6], c: 0x8a9098 },      // 水砲管（朝車尾，末端切齊 −3.63）
  ...
  const DRAGON = [
    ...
    /* 後腿三塊掛 sw（v1.182，使用者：「飛龍也要像獅鷲能做出地面行走」）。
       牠只有兩條腿（前肢就是那對翅膀），所以左右**反相**——bmir 會把 sw 翻號，
       右腿 +0.8 出去左腿就是 −0.8，剛好是一步一步交替（四條腿的牛羊是對角同步的 ±0.5，
       擺幅也只有一半：那是散步，兩條腿的要跨得開才走得動這麼大一隻）。
       髖在大腿的後上角（y −0.20／z −0.60），同一條腿的三塊共用它——
       各繞各的中心轉的話大腿、小腿、腳爪會散成三截（同猴子那三塊手臂）。
       在天上不受影響：擺角是 sin(ph)·sw·**gait**，飛的時候 gait 是 0。 */
    { p: [0.46, -0.42, -0.55], s: [0.36, 0.52, 0.46], c: D_RED, r: [0.50, 0, 0],
      sw: 0.8, pv: -0.20, pz: -0.60 },                                              // 後腿
    { p: [0.50, -0.78, -0.14], s: [0.28, 0.44, 0.32], c: D_RED2, r: [-0.55, 0, 0],
      sw: 0.8, pv: -0.20, pz: -0.60 },
  ...
  /* ── 獅鷲（v1.176）───────────────────────────────────────
     ...
     **v1.177 改成四足**（使用者：「獅鷲應該是四足走」）。v1.176 是站姿——兩條後腿
     站直、前爪抬在胸前，那是「站起來的鷹」不是獅鷲；參考圖上的獅鷲是一隻獅子的身體，
     四隻腳都在地上。所以整份表重畫成水平的獸身：

       前半是鷲     白灰羽的胸與肩、黃色鉤喙的頭往前上方伸、**前腳是黃色的鷹爪**
       後半是獅     黃褐的腰臀、毛腿與肉掌、一條垂在後面的獅尾
       翼根在背上   py 從 2.00 降到 1.62，整片翼壓在背脊上（見 WING_CFG.gryphon）

     四條腿都給 sw（對角同步：前 +0.5、後 −0.5，同牛羊），所以牠**真的會走**——
     v1.176 的前爪是 am（抬起來的手），走起來只有後腿在動。
     原點在腳底（BEAST_FLOOR 算出來是 0），最低的四塊肉掌貼著 y = 0。 */
  ...
  const G_COV = [[0.85, 0.78, 0.32, -0.85], [1.55, 0.76, 0.26, -1.05],
                 [2.20, 0.66, 0.16, -1.00], [2.75, 0.56, 0.04, -0.80],
                 [3.14, 0.42, -0.08, -0.52]].map(q =>
    ({ p: [q[0], 1.62, (q[2] + q[3]) / 2], s: [q[1], 0.10, q[2] - q[3]], c: G_WING2 }));
  const G_PRI = [[1.95, 0.72, -1.05, -1.68], [2.55, 0.62, -1.00, -1.84],
                 [3.05, 0.46, -0.80, -1.56]].map(q =>
    ({ p: [q[0], 1.60, (q[2] + q[3]) / 2], s: [q[1], 0.09, q[2] - q[3]], c: G_WING3 }));
  const GRYPH = [
    /* 獸身是水平的一條：胸（鷲羽）→ 背（翼根壓在這裡）→ 腰 → 臀（獅）。
       前半用白灰羽、後半用黃褐毛，分界就在肩——遠看才分得出「前面是鷹、後面是獅」。 */
    { p: [0, 1.30, 0.62], s: [0.86, 0.86, 0.72], c: G_HD2 },      // 胸（鷲羽）
    { p: [0, 1.16, 0.98], s: [0.60, 0.62, 0.24], c: G_HD },       // 胸前那片白羽
    { p: [0, 1.34, 0.02], s: [0.92, 0.80, 0.90], c: G_BODY },     // 背（翼根在上面）
    { p: [0, 1.24, -0.72], s: [0.86, 0.76, 0.72], c: G_BODY },    // 腰
    { p: [0, 1.10, -1.22], s: [0.76, 0.68, 0.52], c: G_BODY2 },   // 臀
    { p: [0, 0.80, -0.20], s: [0.72, 0.42, 1.40], c: G_BELLY },   // 腹（吊在底下那條淺色）
    /* 脖子往**前上方**伸，而且頭要**抬過翼拱的頂**（v1.177 第一版是 2.49，跟拱頂
       2.42 一樣高，斜前方看過去整顆頭卡在兩片翅膀中間——實際截圖抓到的）。
       現在頭頂 2.71、拱頂 2.42，差 0.29（場上 0.6 格）。 */
    { p: [0, 1.92, 0.90], s: [0.46, 0.66, 0.50], c: G_HD },       // 頸
    { p: [0, 1.60, 0.76], s: [0.74, 0.30, 0.70], c: G_HD2 },      // 頸羽（領圈，壓在肩上）
    { p: [0, 2.46, 1.02], s: [0.50, 0.46, 0.54], c: G_HD },       // 頭
    { p: [0, 2.66, 0.98], s: [0.46, 0.10, 0.46], c: G_HD2 },      // 頭頂平羽（頂到 2.71）
    /* 喙要**鉤**：上喙往前、再掛一塊往下的鉤，下喙短一截縮在裡面。
       只給一塊往前的方塊會變成鸚鵡的短喙，看不出是猛禽。 */
    { p: [0, 2.40, 1.38], s: [0.24, 0.22, 0.30], c: G_BEAK },     // 上喙
    { p: [0, 2.26, 1.48], s: [0.15, 0.18, 0.13], c: G_BEAK },     // 喙鉤
    { p: [0, 2.25, 1.32], s: [0.20, 0.12, 0.26], c: G_BEAK2 },    // 下喙
    { p: [0, 2.48, 1.24], s: [0.26, 0.12, 0.12], c: G_BEAK2 },    // 喙根（鼻孔那一段）
    { p: [0, 1.06, -1.56], s: [0.20, 0.20, 0.34], c: G_BODY2 },   // 獅尾四節
    { p: [0, 0.90, -1.80], s: [0.16, 0.24, 0.18], c: G_BODY2 },
    { p: [0, 0.76, -1.96], s: [0.14, 0.20, 0.16], c: G_BODY2 },
    { p: [0, 0.66, -2.10], s: [0.20, 0.22, 0.20], c: G_WING3 }    // 尾毛
  ].concat(bmir([
    ...
    /* 前腳是**鷹爪**：羽毛大腿 ＋ 黃色跗骨 ＋ 三趾的腳，樞紐取肩（pv 1.30／pz 0.62）。
       sw ＝ +0.5、後腿 −0.5：對角同步，同牛羊那四條腿（擺幅也照牠們的 0.5，
       四條腿各甩快 50 度的話那是奔跑不是走）。 */
    { p: [0.38, 0.98, 0.60], s: [0.30, 0.56, 0.36], c: G_HD2, sw: 0.5, pv: 1.30, pz: 0.62 },
    { p: [0.38, 0.52, 0.66], s: [0.20, 0.40, 0.22], c: G_LEG, sw: 0.5, pv: 1.30, pz: 0.62 },
    { p: [0.38, 0.11, 0.74], s: [0.26, 0.22, 0.44], c: G_LEG, sw: 0.5, pv: 1.30, pz: 0.62 },
    { p: [0.38, 0.07, 1.00], s: [0.11, 0.14, 0.20], c: G_CLAW, sw: 0.5, pv: 1.30, pz: 0.62 },
    { p: [0.38, 0.07, 0.52], s: [0.10, 0.14, 0.14], c: G_CLAW, sw: 0.5, pv: 1.30, pz: 0.62 },
    /* 後腿是**獅腿**：毛大腿 ＋ 小腿 ＋ 肉掌，樞紐取髖（pv 1.24／pz −0.86）。 */
    { p: [0.36, 0.94, -0.86], s: [0.34, 0.66, 0.56], c: G_BODY, sw: -0.5, pv: 1.24, pz: -0.86 },
    { p: [0.34, 0.46, -0.72], s: [0.22, 0.44, 0.28], c: G_BODY2, sw: -0.5, pv: 1.24, pz: -0.86 },
    { p: [0.34, 0.11, -0.62], s: [0.26, 0.22, 0.40], c: G_BODY2, sw: -0.5, pv: 1.24, pz: -0.86 },
    { p: [0.34, 0.07, -0.40], s: [0.11, 0.14, 0.14], c: G_CLAW, sw: -0.5, pv: 1.24, pz: -0.86 }
  ])).concat(bmir(wing(G_COV.concat(G_PRI).concat([
    { p: [0.80, 1.68, 0.20], s: [0.60, 0.22, 0.26], c: G_WING },   // 上臂
    { p: [1.70, 1.66, 0.14], s: [1.10, 0.18, 0.22], c: G_WING },   // 前臂
    { p: [2.70, 1.64, 0.00], s: [1.00, 0.14, 0.18], c: G_WING },   // 腕
    { p: [3.26, 1.62, -0.20], s: [0.30, 0.12, 0.30], c: G_WING3 }  // 翼尖
  ]), WING_CFG.gryphon)));

  /* ── 閒逛的牛羊（v1.154；v1.182 起同一片草地上還有鹿與豬，見下面那一段）──
  ...
  /* ── 閒逛的鹿與豬（v1.182）────────────────────────────────
     使用者：「增加閒逛的動物場上存在數量 & 種類增加 鹿 豬（像牛羊 也要多種版本）」。
     做法整套照牛羊那兩支（骨架共用、一支函式換配色與零件開表），差別在**剪影**——
     四種動物散在同一片草地上，遠看要一眼分得出誰是誰：
       牛  高、方、背平，角短                羊  矮、圓，一團毛
       鹿  腿細長、脖子豎起來、頭抬得比背高  豬  最矮最胖、沒有脖子、鼻盤朝前
     所以肩高一路排開：牛 0.58 ＞ 鹿 0.50 ＞ 羊 0.40 ＞ 豬 0.34，
     而鹿的腿粗細只有牛的三分之二（0.10 對 0.15）——同樣是四條腿，細的那組才像鹿。 */
  const DEER_SH = 0.50, DEER_HIP = 0.50;
  const DE_EY = 0x14100e;
  function deerParts(o) {
    const mid = [
      { p: [0, 0.66, -0.04], s: [0.40, 0.40, 0.80], c: o.hide },      // 軀幹（背頂 0.86）
      { p: [0, 0.68, 0.28], s: [0.38, 0.36, 0.30], c: o.hide },       // 肩胸
      { p: [0, 0.65, -0.42], s: [0.34, 0.34, 0.26], c: o.hide },      // 臀（比肩窄：鹿是前寬後窄）
      { p: [0, 0.50, -0.06], s: [0.34, 0.14, 0.62], c: o.belly },     // 腹（吊在底下那條淺色）
      /* 脖子**豎起來**、頭抬到背上面——這是鹿跟牛最大的差別（牛的頭跟背同高，
         見 cowParts 的頸與頭都在 0.84～0.88）。頭低下去的話遠看就是一頭小牛。 */
      { p: [0, 0.84, 0.40], s: [0.22, 0.32, 0.24], c: o.hide },       // 頸
      { p: [0, 1.00, 0.50], s: [0.19, 0.26, 0.21], c: o.hide },       // 頸（上段）
      { p: [0, 1.12, 0.60], s: [0.20, 0.20, 0.28], c: o.face },       // 頭（頂到 1.22）
      { p: [0, 1.05, 0.77], s: [0.15, 0.14, 0.14], c: o.muzzle },     // 吻
      { p: [0, 1.03, 0.85], s: [0.10, 0.08, 0.05], c: o.nose },       // 鼻頭（深色那一點）
      { p: [0, 0.70, -0.56], s: [0.11, 0.17, 0.10], c: o.tail },      // 尾（翹起來那一撮）
      { p: [0, 0.56, -0.57], s: [0.18, 0.16, 0.07], c: o.tail }       // 臀後那片白
    ];
    if (o.mane) mid.push({ p: [0, 0.80, 0.44], s: [0.29, 0.30, 0.27], c: o.mane });  // 頸下的鬃
    const side = [
      { p: [0.11, 1.17, 0.71], s: [0.06, 0.06, 0.05], c: DE_EY },     // 眼
      /* 耳朵要**大又外張**：鹿耳在剪影上跟角一樣好認，貼著頭畫等於沒畫。 */
      { p: [0.17, 1.21, 0.52], s: [0.17, 0.11, 0.10], c: o.ear, r: [0, 0, 0.42] },
      /* 腿：三節（大腿／小腿／蹄）。同一條腿的三塊**pz 要一樣**，不然三截各繞各的
         關節轉會散開（同猴子那三塊手臂的理由）。擺幅 0.5＝散步，同牛羊。 */
      { p: [0.14, 0.32, 0.26], s: [0.10, 0.38, 0.12], c: o.hide, sw: 0.5, pv: DEER_SH, pz: 0.26 },
      { p: [0.14, 0.11, 0.27], s: [0.09, 0.20, 0.10], c: o.sock, sw: 0.5, pv: DEER_SH, pz: 0.26 },
      { p: [0.14, 0.035, 0.28], s: [0.10, 0.07, 0.11], c: o.hoof, sw: 0.5, pv: DEER_SH, pz: 0.26 },
      { p: [0.15, 0.34, -0.36], s: [0.12, 0.38, 0.15], c: o.hide, sw: -0.5, pv: DEER_HIP, pz: -0.36 },
      { p: [0.14, 0.11, -0.34], s: [0.09, 0.20, 0.10], c: o.sock, sw: -0.5, pv: DEER_HIP, pz: -0.36 },
      { p: [0.14, 0.035, -0.33], s: [0.10, 0.07, 0.11], c: o.hoof, sw: -0.5, pv: DEER_HIP, pz: -0.36 }
    ];
    /* 角：角座 ＋ 兩節往**後上方**掃的主枝 ＋ 一支往前的眉叉，大角的再多一支往外。
       antler＝0 沒角、1 小角、2 大角（同 cowParts 的 hornL：加一款只要換這個數字）。
       **每一節都要細（0.045）又要各自轉角**：第一版是兩塊直立的粗方塊，
       實際截圖看起來是「頭上插了兩片木板」，完全不像角。
       r 的 x 取負＝往後仰（繞 x 轉正是往 +z＝往前倒）。 */
    if (o.antler) side.push(
      { p: [0.08, 1.28, 0.56], s: [0.045, 0.16, 0.045], c: o.antlerC },                 // 角座
      { p: [0.10, 1.46, 0.50], s: [0.045, 0.26, 0.05], c: o.antlerC, r: [-0.35, 0, -0.14] },
      { p: [0.15, 1.58, 0.40], s: [0.045, 0.18 + o.antler * 0.10, 0.05], c: o.antlerC,
        r: [-0.45, 0, -0.24] },
      { p: [0.11, 1.42, 0.68], s: [0.04, 0.16, 0.045], c: o.antlerC, r: [0.55, 0, 0] });  // 眉叉
    if (o.antler > 1) side.push(
      { p: [0.24, 1.60, 0.42], s: [0.04, 0.17, 0.045], c: o.antlerC, r: [-0.20, 0, -0.62] });
    /* 梅花斑要**凸出體側外面**才看得到（同乳牛的側腹斑：軀幹半寬 0.20，斑擺在 0.20）。 */
    if (o.spots) side.push(
      { p: [0.20, 0.79, 0.14], s: [0.03, 0.08, 0.10], c: o.spot },
      { p: [0.20, 0.72, -0.04], s: [0.03, 0.08, 0.10], c: o.spot },
      { p: [0.20, 0.80, -0.18], s: [0.03, 0.07, 0.09], c: o.spot },
      { p: [0.20, 0.70, -0.30], s: [0.03, 0.07, 0.09], c: o.spot },
      { p: [0.18, 0.80, -0.40], s: [0.03, 0.07, 0.09], c: o.spot });
    return mid.concat(bmir(side));
  }
  const DEER = deerParts({                                // 梅花鹿：栗棕底、白斑、小角
    hide: 0xa9743c, belly: 0xe8dcc4, face: 0x97632f, muzzle: 0x6b4a2a, nose: 0x1c1714,
    ear: 0x97632f, tail: 0xf0e8d8, sock: 0x8a5c2c, hoof: 0x2a231f, spot: 0xf2e6cc,
    antlerC: 0x8f7048, antler: 1, spots: 1, mane: 0
  });
  /* 兩款鹿的差別**不能只在角**：站遠了角只有幾個像素，毛色要差一階以上
     （同兩隻羊的教訓——只差一階的話遠看是同一隻）。 */
  const STAG = deerParts({                                // 赤鹿：深褐、頸下有鬃、大角
    hide: 0x7d5432, belly: 0xb99a72, face: 0x5f4025, muzzle: 0x3a2c1e, nose: 0x17120f,
    ear: 0x5f4025, tail: 0xd8c8a8, sock: 0x4f3722, hoof: 0x201a17, spot: 0,
    antlerC: 0xa08256, antler: 2, spots: 0, mane: 0x4a3220
  });

  /* 豬：**沒有脖子**——頭直接接在肩上、鼻盤朝前，背—頭連成一整條。
     羊的頭要探出毛團外面才看得到，豬剛好相反：那條連在一起的線就是豬的剪影。 */
  const PIG_SH = 0.34, PIG_HIP = 0.34;
  const PG_EY = 0x17120f;
  function pigParts(o) {
    /* 背脊要有**高低**、頭要比肩**低一階**：第一版是「軀幹一大塊 ＋ 同色同高的頭」，
       實際截圖是一個長方箱前面插一根鼻子，看不出哪裡是頭。
       現在從臀（0.62）往肩（0.70）升、背脊再墊一條（0.73），頭掉到 0.59——
       側面看是「屁股低、肩背高、頭又低下去」，那條線才是豬。 */
    const mid = [
      { p: [0, 0.44, -0.06], s: [0.50, 0.38, 0.82], c: o.hide },      // 軀幹
      { p: [0, 0.52, 0.16], s: [0.48, 0.36, 0.42], c: o.hide },       // 肩（前面比後面厚）
      { p: [0, 0.66, -0.10], s: [0.38, 0.14, 0.62], c: o.hide },      // 背脊（比軀幹窄，圓一點）
      { p: [0, 0.44, -0.44], s: [0.44, 0.36, 0.24], c: o.hide },      // 臀
      { p: [0, 0.44, 0.48], s: [0.34, 0.30, 0.28], c: o.face },       // 頭（接著肩，沒有頸）
      { p: [0, 0.34, 0.54], s: [0.30, 0.16, 0.22], c: o.face },       // 下顎（豬的臉頰肉）
      { p: [0, 0.40, 0.71], s: [0.25, 0.21, 0.13], c: o.snout },      // 鼻盤（朝前那一片）
      /* 尾巴：家豬**捲一圈**（兩塊接出來的鉤子，不對稱所以不進 bmir），
         山豬是一條直的加一撮毛。 */
      { p: [0, 0.52, -0.58], s: [0.07, 0.12, 0.07], c: o.hide }
    ];
    mid.push(o.curl ? { p: [0.07, 0.58, -0.60], s: [0.11, 0.06, 0.07], c: o.hide }
                    : { p: [0, 0.38, -0.62], s: [0.08, 0.18, 0.08], c: o.bristle });
    if (o.hump) mid.push({ p: [0, 0.68, 0.18], s: [0.42, 0.22, 0.34], c: o.hide });   // 肩峰（山豬）
    if (o.bristle) mid.push(                                          // 背脊上的鬃（山豬）
      { p: [0, 0.79, 0.10], s: [0.09, 0.14, 0.46], c: o.bristle },
      { p: [0, 0.74, -0.28], s: [0.08, 0.12, 0.30], c: o.bristle });
    const side = [
      { p: [0.06, 0.42, 0.77], s: [0.05, 0.05, 0.03], c: o.nose },    // 鼻孔（在鼻盤正面）
      { p: [0.13, 0.50, 0.58], s: [0.05, 0.05, 0.04], c: PG_EY },     // 眼
      /* 耳朵擺在頭與肩的交界、**高過頭頂**：擺在頭側面的話整片被肩擋住看不到。 */
      { p: [0.14, 0.58, 0.42], s: [0.16, 0.10, 0.14], c: o.ear, r: [o.earR, 0, 0.32] },
      /* 腿短又粗：兩節就夠（同牛羊），細節多了在這個高度也看不到。 */
      { p: [0.18, 0.20, 0.26], s: [0.13, 0.28, 0.15], c: o.hide, sw: 0.5, pv: PIG_SH, pz: 0.26 },
      { p: [0.18, 0.05, 0.27], s: [0.14, 0.10, 0.16], c: o.hoof, sw: 0.5, pv: PIG_SH, pz: 0.26 },
      { p: [0.18, 0.20, -0.34], s: [0.14, 0.28, 0.16], c: o.hide, sw: -0.5, pv: PIG_HIP, pz: -0.34 },
      { p: [0.18, 0.05, -0.34], s: [0.15, 0.10, 0.16], c: o.hoof, sw: -0.5, pv: PIG_HIP, pz: -0.34 }
    ];
    if (o.tusk) side.push(                                            // 獠牙（山豬，要伸出鼻盤外面）
      { p: [0.08, 0.44, 0.80], s: [0.045, 0.15, 0.05], c: o.tusk, r: [-0.55, 0, -0.28] });
    return mid.concat(bmir(side));
  }
  const HOG = pigParts({                                  // 家豬：粉紅、耳朵往前垂、尾巴捲一圈
    hide: 0xe4a79c, face: 0xdd9c92, snout: 0xc97b74, nose: 0x8b544f, ear: 0xd99a92,
    hoof: 0x3a2f2a, bristle: 0, tusk: 0, hump: 0, curl: 1, earR: 0.55
  });
  const BOAR = pigParts({                                 // 山豬：深褐、肩峰、背上一排鬃、獠牙、耳朵立著
    hide: 0x5a4a3a, face: 0x4a3c2f, snout: 0x3a2f26, nose: 0x201a15, ear: 0x4a3c2f,
    hoof: 0x1e1a16, bristle: 0x2c241c, tusk: 0xe8e0cc, hump: 1, curl: 0, earR: -0.35
  });
  ...
  /* ── 巨人（v1.192）───────────────────────────────────────
     ...
     **原點在腳底**（同猴子與獅鷲，BEAST_FLOOR 算出來是 0），模型高整整 5.00——
     配規則那邊的 GIA_SC 3.0 就是場上 15 格（小人 2.26 格的 6.6 倍，跟吉薩大金字塔
     頂端 13.5 格齊高，使用者選的）。頭只佔 0.55（全身的九分之一）：
     **「頭小、肩寬、腿長」才是巨人**，照真人比例（七頭身）縮放出來的只是一個放大的小人。

     兩條腿所以走路左右**反相**（右腿 sw +1，bmir 翻成 −1），同飛龍不同牛羊。
     腿上那七塊全掛 kk＝「這一塊要跟著踢」：攻擊是腳踢，右腿繞髖往前上方抬
     （見 putBeasts 的 m.kick，跟右手的 m.arm 同一個做法）。 */
  const GIA_SH = 3.95, GIA_HIP = 2.30;          // 肩／髖關節的高度（手腳繞它轉）
  /* 腳掌那一塊的位置。**拉出來當常數**是因為規則那邊要拿它算「踹到哪一點」
     （見下面的 giantFoot）——表上跟判定用的必須是同一組數字。 */
  const GI_SOLE = [0.31, 0.11, 0.14];
  /* 前綴是 GI_ 不是 G_：G_ 那一組是獅鷲的（G_EYE 已經有人用了） */
  const GI_MUS = 0xa34036, GI_MUS2 = 0xb85446, GI_MUS3 = 0x74271f,   // 肌肉：主色／亮面束／深溝
        GI_TEN = 0xdcc2a8, GI_FACE = 0xd8c0a4, GI_FACE2 = 0xc9a98c,  // 筋膜腱／顱／臉盤
        GI_TEETH = 0xf0e8d6, GI_SOCK = 0x2a1a14, GI_EYE = 0xe6d9b8;  // 牙／眼窩／眼球
  const GIANT = [
    /* 軀幹一節一塊，寬度落差就是剪影：胸 1.06 → 腰 0.82 → 骨盆 0.84，肩上再加斜方肌 0.90 */
    { p: [0, 2.52, 0],     s: [0.84, 0.52, 0.52], c: GI_MUS },     // 骨盆
    { p: [0, 2.95, 0.02],  s: [0.82, 0.40, 0.50], c: GI_MUS },     // 下腹
    { p: [0, 3.32, 0.02],  s: [0.90, 0.36, 0.52], c: GI_MUS },     // 上腹
    { p: [0, 3.12, 0.30],  s: [0.05, 0.74, 0.03], c: GI_MUS3 },    // 腹中線（腹肌靠溝分塊，不靠顏色）
    { p: [0, 3.44, 0.30],  s: [0.60, 0.05, 0.03], c: GI_MUS3 },    // 腹橫溝三條
    { p: [0, 3.20, 0.30],  s: [0.64, 0.05, 0.03], c: GI_MUS3 },
    { p: [0, 2.96, 0.30],  s: [0.58, 0.05, 0.03], c: GI_MUS3 },
    { p: [0, 3.72, 0.02],  s: [1.06, 0.44, 0.62], c: GI_MUS },     // 胸廓（最寬的一塊）
    { p: [0, 3.94, 0.30],  s: [0.06, 0.28, 0.04], c: GI_MUS3 },    // 胸溝
    /* 背闊肌與臀要**凸到身體後緣外面**：側面看沒有這兩塊凸起的話，
       從頭到腳是一條直邊——第一版出圖就是一塊紅色的板子（看圖才發現的）。 */
    { p: [0, 3.86, -0.30], s: [0.92, 0.40, 0.14], c: GI_MUS2 },    // 背闊肌
    { p: [0, 3.55, -0.32], s: [0.06, 0.90, 0.03], c: GI_MUS3 },    // 背脊溝
    { p: [0, 2.44, -0.30], s: [0.76, 0.46, 0.26], c: GI_MUS2 },    // 臀
    { p: [0, 4.12, -0.02], s: [0.90, 0.26, 0.48], c: GI_MUS2 },    // 斜方肌（肩上那一坡）
    { p: [0, 4.34, 0.02],  s: [0.36, 0.34, 0.36], c: GI_MUS },     // 頸（沒有皮，就是一束肌肉）
    { p: [0, 4.32, 0.20],  s: [0.16, 0.30, 0.04], c: GI_TEN },     // 喉前那兩條腱
    { p: [0, 4.68, 0.06],  s: [0.46, 0.44, 0.46], c: GI_FACE },    // 顱
    { p: [0, 4.925, 0.04], s: [0.40, 0.15, 0.40], c: GI_FACE },    // 顱頂（頂到 5.00）
    { p: [0, 4.72, 0.27],  s: [0.38, 0.30, 0.06], c: GI_FACE2 },   // 臉盤（下緣收到 4.57，牙齒才露得出來）
    { p: [0, 4.66, 0.310],  s: [0.10, 0.14, 0.03], c: GI_MUS3 },   // 鼻窩——**沒有鼻子，是一個凹**
    /* 咧開的那排牙（參考圖的招牌）。踩到兩次才對：
       ① 第一版三塊全給淡色，整個下半臉糊成一團米色，牙齒完全看不見 → 下顎改回紅肌肉。
       ② 第二版兩排牙中間只夾 0.035 的暗縫（場上 0.1 格），出圖還是一條白帶不是一張嘴
          → 改成**先擺一塊暗的口腔、兩排牙貼在它前面**，中間空 0.085（場上 0.26 格）
          就自然是那一條黑縫。牙還要凸出臉盤前緣（臉盤 0.30、牙 0.325），
          同大劍那條「刃口比刃薄又同一個中心，整片被遮掉」。 */
    { p: [0, 4.450, 0.275], s: [0.34, 0.20, 0.05], c: 0x3a1a14 },  // 口腔（襯在牙後面的暗）
    { p: [0, 4.525, 0.300], s: [0.33, 0.07, 0.05], c: GI_TEETH },  // 上排牙
    { p: [0, 4.375, 0.300], s: [0.31, 0.06, 0.05], c: GI_TEETH },  // 下排牙
    { p: [0, 4.310, 0.150], s: [0.32, 0.14, 0.28], c: GI_MUS }     // 下顎（**紅肌肉**，不是淡色）
  ].concat(bmir([
    /* 眼窩**第一版開太大**（0.155 寬配一顆 0.10 的白眼球），出圖像戴了一副蛙鏡。
       現在是「小小一顆淡色的眼球陷在一個暗洞裡」：洞 0.12、眼球 0.055。 */
    { p: [0.14, 4.765, 0.285], s: [0.12, 0.12, 0.05], c: GI_SOCK },   // 眼窩（深陷的一個洞）
    { p: [0.14, 4.755, 0.305], s: [0.055, 0.055, 0.03], c: GI_EYE },  // 眼球（要凸出眼窩外才看得到）
    { p: [0.25, 4.66, 0.08],   s: [0.05, 0.30, 0.34], c: GI_MUS2 },   // 顳肌（臉側那條紅肌）
    /* 胸肌要**凸出胸廓前緣**才有胸（胸廓前緣 0.33、這一塊到 0.39）：
       第一版只凸 0.04，正面看整片胸是平的。 */
    { p: [0.34, 3.80, 0.31],   s: [0.44, 0.32, 0.16], c: GI_MUS2 },   // 胸肌
    { p: [0.34, 3.62, 0.30],   s: [0.42, 0.06, 0.12], c: GI_MUS3 },   // 胸肌下緣
    { p: [0.42, 2.52, 0.02],   s: [0.28, 0.44, 0.46], c: GI_MUS2 },   // 髖外側
    { p: [0.34, 2.62, 0.26],   s: [0.09, 0.44, 0.04], c: GI_TEN },    // 鼠蹊那條白腱
    /* 手臂七塊接起來（三角肌／上臂／肘腱／前臂／腕腱／掌／指），**全部繞肩關節轉**：
       各轉各的中心會散成七截（見檔頭那段註解，猴子踩過同一件事）。 */
    { p: [0.70, 3.82, 0.02],  s: [0.36, 0.40, 0.44], c: GI_MUS,  am: 1, pv: GIA_SH },
    { p: [0.74, 3.38, 0.04],  s: [0.30, 0.52, 0.34], c: GI_MUS,  am: 1, pv: GIA_SH },
    { p: [0.74, 3.09, 0.02],  s: [0.26, 0.10, 0.28], c: GI_TEN,  am: 1, pv: GIA_SH },
    /* 手要垂到大腿中段（參考圖上就是那個長度）：指尖落在 1.89，大腿是 1.35～2.29。 */
    { p: [0.73, 2.74, 0.04],  s: [0.27, 0.54, 0.29], c: GI_MUS2, am: 1, pv: GIA_SH },
    { p: [0.72, 2.44, 0.05],  s: [0.22, 0.08, 0.24], c: GI_TEN,  am: 1, pv: GIA_SH },
    { p: [0.72, 2.25, 0.07],  s: [0.25, 0.30, 0.27], c: GI_MUS,  am: 1, pv: GIA_SH },
    { p: [0.72, 2.00, 0.11],  s: [0.24, 0.22, 0.24], c: GI_MUS3, am: 1, pv: GIA_SH },
    /* 腿八塊，同樣全部繞髖轉。kk＝踢的時候跟著抬（只有右腿那一條真的會動，
       左腿是 bmir 翻出來的 sw −1，見 putBeasts 的 b.sw > 0）。

       **sw 給 0.62 不給猴子那個 1**：擺角是 sin(ph)·sw·gait，gait 走起來是 0.85，
       所以 1 是 ±49°——那是踢正步不是走路。0.62 ＝ ±30°，一步跨
       2 × 6.90(髖到腳底，場上) × sin(0.527) ＝ 6.9 格，剛好是牠自己的腿長。 */
    { p: [0.31, 1.82, 0.02],  s: [0.44, 0.94, 0.50], c: GI_MUS,  sw: 0.62, kk: 1, pv: GIA_HIP },  // 大腿
    { p: [0.31, 1.76, 0.27],  s: [0.10, 0.82, 0.04], c: GI_TEN,  sw: 0.62, kk: 1, pv: GIA_HIP },  // 大腿前那條白腱
    { p: [0.31, 1.24, 0.04],  s: [0.38, 0.22, 0.42], c: GI_TEN,  sw: 0.62, kk: 1, pv: GIA_HIP },  // 膝
    { p: [0.31, 0.82, -0.10], s: [0.36, 0.68, 0.42], c: GI_MUS,  sw: 0.62, kk: 1, pv: GIA_HIP },  // 小腿（腓腸肌要往後凸）
    { p: [0.31, 0.80, 0.19],  s: [0.10, 0.62, 0.04], c: GI_TEN,  sw: 0.62, kk: 1, pv: GIA_HIP },  // 脛前腱
    { p: [0.31, 0.36, -0.02], s: [0.26, 0.28, 0.26], c: GI_TEN,  sw: 0.62, kk: 1, pv: GIA_HIP },  // 踝腱
    { p: GI_SOLE,             s: [0.38, 0.22, 0.64], c: GI_MUS,  sw: 0.62, kk: 1, pv: GIA_HIP },  // 腳掌
    { p: [0.31, 0.07, 0.48],  s: [0.36, 0.14, 0.12], c: GI_MUS3, sw: 0.62, kk: 1, pv: GIA_HIP }   // 腳趾
  ]));
  /* 這一腳踹到哪一點（**模型座標**，原點在腳底、+z 是正面、還沒乘 sc）。
     規則那邊要拿它決定「炸在哪、火從哪裡起」，而畫面上腳掌就是繞髖轉這個角度畫出來的
     ——同 SWORD_TIP／WAND_TIP／DIG_TIP：**畫出來的跟判定用的必須是同一份數字**，
     各寫一份的話會變成「腳踹在這裡、積木炸在那裡」。
       kick 0～1（規則那邊的 m.kick）、lean 身體往後仰幾度（m.spin 的絕對值）。
     轉法照抄 putBeasts：先繞髖（pv ＝ GIA_HIP、關節 z ＝ JOINT_Z）轉 −BEAST_KICK·kick，
     再繞原點（腳底）轉後仰那一下。 */
  function giantFoot(kick, lean) {
    const ang = -BEAST_KICK * (kick === undefined ? 1 : kick);
    const dy = GI_SOLE[1] - GIA_HIP, dz = GI_SOLE[2] - JOINT_Z;
    const c = Math.cos(ang), s = Math.sin(ang);
    let y = GIA_HIP + dy * c - dz * s, z = JOINT_Z + dy * s + dz * c;
    if (lean) {                                   // 後仰是繞 x 轉（同 putBeasts 的 m.spin）
      const cl = Math.cos(-lean), sl = Math.sin(-lean);
      const y2 = y * cl - z * sl; z = y * sl + z * cl; y = y2;
    }
    /* x 要一起給：踢的是**右腿**，腳掌本來就偏在中線右邊 0.31（場上 0.93 格）。
       繞髖轉與後仰都只動 y／z，所以 x 原封不動——但呼叫端不能當它是 0，
  ...
  /* 每一種的模型範圍。破壞工具打得到牠們之後（v1.146），規則那邊要拿這三個數字擺姿勢，
     所以照造型表算出來、不寫死——改造型時不必記得回來改常數。
       ...
       side  側躺（繞 z 轉 ±90°）時世界高度變成模型的 x，所以要抬的是「最寬的那一塊」
             ...
             **side 要把 r 轉過的那幾塊算寬一點**（v1.182）：斜著擺的方塊比 p ± s/2 寬，
             照原本的算法鹿側躺會陷進草皮（實測梅花鹿 0.026、赤鹿 0.081 格——耳朵斜 0.42、
             角的側叉斜 0.62，是〈每一款動物站著、側躺、躺著壓火都貼著草皮〉那條抓到的）。
             繞 x 轉不影響 x 的投影，繞 y／z 才會：照 three 的 XYZ 順序，旋轉矩陣第一列是
             [cos y·cos z, −cos y·sin z, sin y]，三軸各自的投影加起來就是這一塊真正的寬度。
             牛羊與獅鷲那幾款一格都沒變（牠們沒有繞 y／z 轉的部位），飛龍的 side 沒人讀
             （只有 m.side 的才側躺，龍不是）。 */
  ...
  for (const k in BEASTS) {
    ...
    for (const b of BEASTS[k]) {
      ...
      if (!b.wg) {
        const ry = b.r ? b.r[1] : 0, rz = b.r ? b.r[2] : 0;
        const w = Math.abs(Math.cos(ry) * Math.cos(rz)) * b.s[0] +
                  Math.abs(Math.cos(ry) * Math.sin(rz)) * b.s[1] +
                  Math.abs(Math.sin(ry)) * b.s[2];
        xhi = Math.max(xhi, Math.abs(b.p[0]) + w / 2);
      }
    }
  ...
  /* 踢到底是幾度（v1.192 巨人，規則那邊給 0～1 的 m.kick）。跟 BEAST_RAISE 同一個用途，
     只是掛在腿上。這個值收過兩次，**都是使用者看圖定的**：
       1.55 → 1.30  接近水平，是一記芭蕾的高踢腿，不像十五格重的東西在踹
       1.30 → 0.80  使用者：「腳踢可以不用踢到那麼高 大概現在的一半就好」

     「多高」是**量出來的**（從畫出去的矩陣讀腳掌那一塊的世界座標），不是算的：
     踢的世界角度是「踢角 ＋ 身體後仰」，照公式算會漏掉後仰那一段。
     場上（×3、後仰 0.12）腳掌中心 1.30 弧度時在 6.2 格高、5.8 格遠，
     0.80 弧度時在 **3.1 格高、4.7 格遠**——高度剛好收一半，前伸只少兩成
  ...
  function putBeasts(list) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      for (let k = 0; k < BEAST_PARTS; k++) {
        ...
        let ang = 0;
        if (b.sw) {
          ang = Math.sin(m.ph || 0) * b.sw * (m.gait || 0);
          /* 踢腿（v1.192 巨人）：**只有右腿那一條**（sw > 0；左腿是 bmir 翻出來的 −1）
             照 m.kick 繞髖往前上方抬。負角＝往前，所以是減的——跟右手的 m.arm 同一條式子。 */
          if (b.kk && b.sw > 0 && m.kick)
            ang -= (m.kickA === undefined ? BEAST_KICK : m.kickA) * m.kick;
        } else if (b.am) {
