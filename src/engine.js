...
const ENG = (function () {
  ...
  /* 每個小人的部位數，要跟 BODY 的長度一模一樣。7 個身體部位 ＋ 藍圖 ＋ 聊天泡泡兩塊
     ...
     ＋ v1.121 表情圖示的五塊（通用小方塊，照圖樣表重擺，見 EMO_ART）。
  ...
  /* ── 頭上的表情圖示（v1.121）─────────────────────────────────
     ...
     做法是**五塊通用的小方塊**（EMO_SLOT），位置／大小／顏色每幀照圖樣表重算——
     不是一種表情一組部位。四種表情共用同一組，所以再加第五、第六種也不會多一塊部位
     （一塊部位 = 80 個 instance）。一個圖樣最多五塊：夠畫出問號那種轉三個彎的形狀，
     再多就不是「一眼認得出」的圖示了。
  ...
  const EMO_ART = {
    ...
    /* 愛心：上面兩瓣 ＋ 一條寬帶 ＋ 收窄 ＋ 尖，voxel 的心就是這樣一階一階收下來的。
       兩瓣中間**一定要留缺口**（各 0.14 寬、擺在 ±0.11，中間留 0.08）：兩瓣接起來就
       只是一塊比較寬的方塊，整個圖示會讀成一個「T」（v1.121 第一版就是這樣，看圖看出來的）。 */
    heart: { c: 0xff5f8a, r: [[-0.12, 0.415, 0.14, 0.17], [0.12, 0.415, 0.14, 0.17],
                              [0, 0.265, 0.38, 0.13], [0, 0.15, 0.24, 0.10],
                              [0, 0.055, 0.11, 0.11]] },
    /* 生氣：兩道往中間壓下來的怒眉 ＋ 下面兩顆眼睛，也就是一張皺著眉的臉。
       斜角是這兩塊才有的，其餘三種都是正的方塊，所以遠遠看到「斜的」就知道是生氣。
       這個是拍圖比出來的（v1.121，比過四版）：
         · 漫畫的怒氣符號（∧ 疊 ∨、四道斜槓炸開）在這個尺寸下讀成一個中空的菱形，
           而井字（髒話符號）雖然清楚，但要先知道那個典故。
         · 眉要壓在**眼睛的外側**（都在 ±0.13）：眼睛往中間收的話四塊會連成一個大 V。 */
    anger: { c: 0xe8342a, r: [[-0.13, 0.34, 0.26, 0.10, -0.5], [0.13, 0.34, 0.26, 0.10, 0.5],
                              [-0.13, 0.14, 0.12, 0.12], [0.13, 0.14, 0.12, 0.12]] }
  };
