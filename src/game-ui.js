...
/* 躺在地上的碎料，畫的時候整批往下沉一點點（v1.160.1）。
   使用者：「碎料有黑有白時轉動攝影機 會有白黑抖動」。

   落定的碎料**全部是軸向的**（startSnap 把旋轉轉到 90 度的倍數）、而且高度
   **完全一樣**（halfY 對軸向的積木算出來就是 HB ＝ 0.47），可是 separate 只保證
   兩塊的中心距離 ≥ BS——斜著相鄰的兩塊在距離剛好 BS 時，x 與 z 各重疊 0.27，
   於是那一小塊重疊區裡，兩張頂面是**同一個平面**。深度緩衝分不出誰在前面，
   鏡頭一轉就換一個贏；一塊焦炭配一塊白積木，看起來就是黑白亂跳。
   實測（3000 塊黑白相間的碎料，鏡頭每次只轉 0.023°——真的邊緣只移動 0.03 像素，
   不可能翻轉）：亮度翻轉 **884.8 → 29.1 個像素／幀**（共 102.4 萬像素）。
   同一坨換條件確認過不是陰影：`shadow.bias` 開到爆是 816.6、太陽完全不投影是 874.4，
   幾乎沒動；而同一坨碎料只把顏色改成全白就掉到 12 個（那是正常的邊緣移動）。

   往**下**沉不往上抬：沉進地面看不見（地面頂就在 y = 0），抬起來會在底下露出縫。
   8 階 × 4mm：**階距**才是關鍵，它要大過深度緩衝在那個距離的解析度——階數加到
   32 階沒有用（51 個），階距降到 2mm 就退回 72 個（鏡頭拉到最遠時）。
   28mm ＝ 一塊的 3%，鏡頭貼最近時也只有 1.8 像素。
   分階直接用池子裡的編號：碎料的位置跟編號無關，等於隨機分配；池子重排
   （dropBlocks）會讓某塊換一階，那是 0.5 像素的事。
   只有「躺在地上的自由碎料」要沉——蓋好的建築是照格子擺的、彼此留 0.06 的縫，
   本來就不共面。 */
const REST_SINK = 0.004;
function draw() {
...
function onUp(e) {
  ...
  if (!isClick) return;
  audio();                                // 使用者互動後才允許出聲
  /* 大劍的第二下**不走 pick**（v1.197，使用者：「是兩下很難決定切的角度」）：
     那一下給的是**角度**不是位置，所以它要的是螢幕上那條線，不是游標底下那塊表面
     ——射線 × 通過第一點的那面垂直平面就是刃掃過去的線（見 game-tools 的 swordPoint）。
     順帶把兩件事修掉：螢幕往上比不再變成「往遠推」，而且**點天空也算數**
     （pick 打不到東西會回 null，下面那行就直接 return 了）。
     第一下照舊走 pick：它要真的打到表面才有位置與高度可記。 */
  if (tool === 'sword' && aim) { useTool(swordHit(x, y)); return; }
