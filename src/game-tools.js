...
const MET_FIRE = MET_S / 2;     // 火頭的散布半徑倍率（×2）；單顆大小不跟著放大，見 stepMeteors
/* 拖著的火與煙各自的閘門（v1.200.1）。兩個都是「同時幾顆隕石一起分」——
   配額再照 √n 收（同龍捲風的塵霧與碎料的火苗），六顆一起掉也不會各自吃滿。
   火：舊的閘門是 HOT_MAX − 30 ＝ 190，而一顆自己就要 145（實測），兩顆就開始被切。
   煙：舊的閘門是 380，**而完工慶祝的彩帶自己就佔掉 346**（實測 dust 一路頂在 376～378）
   ——「顆數往上」在那個場景根本擠不進去，尾煙會斷成一節一節的。
   兩個數字都留在「不會把蘑菇雲的份吃掉」的範圍內（雲的閘門是 1550／1350、
   引擎 MAXDUST 2200；火那邊引擎 MAXFIRE 1280）。 */
const MET_HOT_CAP = 340, MET_SMOKE_CAP = 700;
...
function stepMeteors(dt) {
  ...
  for (let i = meteors.length - 1; i >= 0; i--) {
    ...
    /* 這一幀走過的那一段（拖尾要沿著它撒、也要順著它拉長）。
       sl 是走了多遠：正常一幀跑五個單位，不會是 0，但方向要拿來當單位向量，
       所以還是擋一下——不是單位向量的話引擎那邊整條線會歪掉。 */
    const sx = m.x - px, sy = m.y - py, sz = m.z - pz;
    const sl = Math.hypot(sx, sy, sz);
    const dx = sl > 1e-4 ? sx / sl : 0, dy = sl > 1e-4 ? sy / sl : 0, dz = sl > 1e-4 ? sz / sl : 0;
    const share = Math.sqrt(meteors.length);       // 同時幾顆就分掉多少配額（見 MET_HOT_CAP）
    m.em += dt * 660 / share;
    while (m.em >= 1) {
      m.em--;
      if (hot.length > MET_HOT_CAP) break;
      /* 兩種火（v1.200.1 整批變細，見 開發筆記〈隕石的火與煙再細一級〉）：
         ①「火頭」生在石頭四周（不是沿著路徑），整圈把石頭包起來。全部平均撒在線段上
           的話，畫面是「一塊石頭後面跟著一串小點」，看不出石頭本身在燒；
           生在石頭正中央也不行——那顆立方體會把火擋在後面。
         ②「拖尾」沿著這一幀走過的線段撒，而且**順著飛行方向拉成短條**（ln，
           跟爆炸的火星、投石機火花、煙火拖線同一個做法）：一顆一顆的小方塊
           連不成一條尾巴，拉長之後才是「拖著火」。
         **散布半徑**跟著石身等比走（MET_FIRE，v1.151.1 是 ×2）：寫死的話那圈火會縮在
         石頭裡面，看起來石頭沒在燒。**單顆大小不跟著放大**（v1.200.1 改）——
         等比放大是 v1.151.1 留下來的，火頭那批因此每顆 2.0～3.6 格（比一塊積木大兩三倍），
         使用者說的「火紅方塊」就是它；火本來就該小、密、短命。 */
      const head = Math.random() < 0.45;
      ...
      const h = {
        ...
        rx: Math.random() * 6, ry: Math.random() * 6,
        s: head ? rr(0.22, 0.46) * MET_FIRE : rr(0.12, 0.3),
        life: head ? rr(0.1, 0.22) : rr(0.16, 0.4),
        g: -1.6, grow: head ? 1.05 : 1.04, cool: rr(0.2, 0.42),
        cr: 1, cg: rr(0.5, 0.84), cb: rr(0.06, 0.22), to: [0.55, 0.1, 0.02]
      };
      if (!head && sl > 1e-4) { h.dx = dx; h.dy = dy; h.dz = dz; h.ln = rr(0.6, 1.9); }
      hot.push(h);
    }
    /* 尾煙：火後面拖一條深色的煙，尾巴才有長度感（往上飄，所以 g 給負的）。
       v1.200.1 一起變細，照〈顆粒再細一級〉那條：**顆數往上、單顆往下**
       （每秒 44 → 110 團、單顆 0.55～1.35 → 0.3～0.75），總覆蓋度幾乎不動。 */
    m.smoke += dt * 110 / share;
    while (m.smoke >= 1) {
      m.smoke--;
      if (dust.length > MET_SMOKE_CAP) break;
      const u = Math.random();
      dust.push({
        x: px + sx * u + rr(-0.5, 0.5),
        y: py + sy * u + rr(-0.5, 0.5),
        z: pz + sz * u + rr(-0.5, 0.5),
        vx: rr(-1, 1), vy: rr(0.5, 2), vz: rr(-1, 1),
        rx: Math.random() * 6, ry: Math.random() * 6,
        life: rr(0.9, 2), s: rr(0.3, 0.75), c: rr(0.22, 0.38), g: -0.5, fade: 1.8
      });
    }
...
/* 第一下記位置與高度，第二下才揮。哪裡都能點（v1.164 使用者：「不再限定一定要點建築
   ...
   出一行提示、第一點留著等你再點一次建築）。 */
function aimSword(point) {
  if (!aim) {
    aimFirst(point, SW_AIM_R, SW_AIM_C);
    /* sy＝點到的那個高度。face＝這一把的標記要浮在空間中、正對鏡頭
       （v1.196，其餘五把用同一組環但不給這個欄位，畫法一個像素都不變）。 */
...
/* 穿門那一段結束了，回原本在做的事（v1.190.2）。
   **目標一定要重算**：走人（go）那一段的目標是「從牠當時站的地方徑向往外」那個場外點，
   而繞門出城之後那個點還在城的另一邊——於是 wallSplits 又成立，牠再穿門走回城裡，
   撞到對面那道牆又去走門，如此往復。實測 400 秒走了 859 格還在城裡，週期 53 秒
...
function gateBack(m) {
  ...
  else if (m.st === 'fun') idleSpot(m);      // 逛的目標被借走了，重挑一個
  // come 下一幀自己把目標設回工地中心；near 不吃 m.tx／m.tz
}
...
/* 前面真的被城牆擋住了嗎（v1.186）。**還沒蓋起來、或者剛好有個缺口就直直走過去**
   （使用者：「主要是能走過去就走(例如還沒蓋起來) 都蓋起來就走門 走不過就破牆而入」）。

   v1.186~v1.194 是「wallSplits（幾何方框）＋往前探 WALL_EYE 1.5 格有沒有踩到牆身」。
   那個組合會說謊兩次：幾何方框不知道牆蓋到哪裡，而 1.5 格的探針只看得到正前方——
   **旁邊十幾格的大洞牠看不到**，直線一撞到有砌的那一段就繞去走門了。
   實測（牆半徑 27、缺口在 +z 牆 x≈14 那一段，猴子從 (26, 35) 進場）：
   繞門 472 幀（23.6 秒）、走了 91.7 格才進城，而直線只有 43.6 格。

   v1.195 換成 wallBlocked：拿整條直線去跟**已經砌好的那幾格**做相交，誠實。
   探針那一關也跟著拿掉——**改成遠遠就決定**：當初「不預先繞路」是因為幾何判斷會把
   缺口誤判成擋住，判斷誠實之後那個理由就沒了，而留著它等於白走到牆邊再掉頭。 */
const wallAhead = (m, tx, tz) => wallBlocked(m.x, m.z, tx, tz);
/* 牆**就在腳邊**了嗎：往要去的方向探 WALL_EYE 格（牆 3 格厚，貼著外面那一面時
   這一點就在牆身裡）。v1.186~v1.194 這一關是跟「走不走得過」綁在一起的，v1.195 拆開：

     走不走得過（要不要繞）  → wallAhead，**遠遠就決定**，判斷誠實所以不必走到牆邊試
     要不要就地破牆而入      → 這一支，**一定要走到牆邊**

   拆開的理由是實測踩到的：沒有開口可繞的時候，牠會轉進 near 去動手，而 near 挑的是
   `nearHome` ——**離牠最近的那一棟**。在城裡半路就轉過去的話那一棟是城內的房子，
   牠就去燒房子了，擋路的那道牆一塊都沒拆（實測開工中、四座門樓都沒了那一局：
...
/* 爬起來那一段**朝向要限速轉**（v1.202.2）。躺著的時候 hurtBeast 會把底下整段跳掉、
   ...
   near 段的 `m.a = Math.atan2(dx, dz)`、獅鷲是 grAim 的 `m.a = m.a0`——而這時候身體
   還躺著（實測躺平角還在 −79°），躺平的身體就被原地甩到另一個方向去。
   實測（單發箭打中之後追 300 幀）那一幀 **巨人轉 61.5°、獅鷲 117.7°、黑獼猴 111.3°**，
   畫出去那一團的重心一幀跳 **8.98 格**（巨人躺平時重心離腳底 8.7 格，轉身就被放大）。
   同一個道理在飛龍盤旋那裡處理過——不用參數式而是每幀限速轉，理由寫著「進場那一刻
   不會有『朝向瞬間跳到切線上』的瞬移感」（見 開發筆記〈圓弧是「轉出來的」，不是照參數式擺位置〉）。
   **包在外面**，不去改 near／aim／walk 那幾處賦值：那幾處散在好幾段，一處一處改是
   會漏的那種改法，而「還沒站直」在這裡只要問 m.lie（v1.202.1 起那個旗標會一路撐到
   躺平角收完，見 hurtBeast）。
   **只動看起來的方向，規則面一個字都沒變**：near 段前進走的是 dx／dz 不是 m.a、
...
function stepBeast0(m, dt) {
  ...
  /* 卡住了就脫困（v1.190.2，使用者：「小人 牛羊 猴子這類盡量同一套走位判定」）。
     整套跟小人共用同一支 stuckWatch（腿在擺卻沒前進 → 先把壓在人家外框裡的目標挪出來，
     再撐一下就穿透幾秒），動物身上本來就有那組欄位（ghost／sx／sz／stk），
     pushOutHome 與 blockHome 也早就認 ghost——**只差沒人替牠們計時**。
     城牆圍起來之後牠們會遇到兩種小人碰不到的殘局：門口那一點被城內的房子壓住
     （實測 7952 幀動不了），還有牛羊沒有攻擊手段、沒門可繞時只能穿出去。
     擺在最前面：下面每一條分支都會 return（同 updWorker 裡的位置）。 */
  stuckWatch(m, dt);
  ...
  /* **正在穿城門的不要打斷**（v1.190.2）：穿門是一段不可分割的位移（走到門口那一點 →
     穿過去），而這一條會把牠拉回 go，go 那一段下一行又發現被牆擋住、再把牠推回 gate——
  ...
  if (m.st === 'fun') {
    ...
    if (m.bad) {
      ...
      if (!t) { m.bad = 0; m.home = 0; }
        ...
        /* 站位落在工地圈內就改站房子的**外側**（v1.195，見 開發筆記〈順帶：一條真的
           卡住的〉）。站位是「從房子中心往牠的方向推」，牠比房子更靠近場心時那一點會
           落在工地圈內，接著被 strollTo 開頭推到圈上——**那個點跟「房子旁邊」已經
           沒關係了**，牠在圈上追著一個跟房子無關的點跑（實測盯上的那一間 r = 28.4、
           算出來的站位 r ≈ 13.3、被推到 20.11 ＝ siteR + KEEP，牠追了 887 幀整趟白來）。 */
  ...
  if (m.st === 'near') {
    ...
    if (adv < 0.05 || footBlocked(ex, ez) || homeFoot(ex, ez)) {
      ...
      if (blk && blk.wall && wallBlocked(m.x, m.z, aim.x, aim.z)) {
  ...
  if (wallAhead(m, m.tx, m.tz)) {
    if (gateNeed(m, m.tx, m.tz)) return false;
    /* 沒門可繞（門樓被打爛了）：**會動手的就地拆牆出去**（v1.190.2，跟進城那一段對稱，
       使用者選的「會動手的拆牆、其他穿過去」）。牛羊沒有攻擊手段（DOOM_ACT 裡沒有牠們
       那幾款），交給 stuckWatch 穿出去。不給出路的話牠就對著牆磨到這一輪結束——
       實測 8000 幀（400 秒）定在牆內側 (1.5, −25.5) 一步都沒動。 */
    if (canFight(m) && wallFoot(m, m.tx, m.tz)) {     // 走到牆邊才動手（v1.195，見 wallFoot）
      m.home = 1; m.st = 'near'; m.leg = 0; return false;
    }
  }
...
/* 爬起來那一段最快轉幾弧度／秒（v1.202.2，見 stepBeast 那層包裝）。
   **直接借獅鷲站在地上轉身那個值**，不另外抓一個數字：同樣是「站在草皮上轉身」
   這件事，沒有理由兩套（而且抄一份數字過來的話，那邊改了這邊不會跟著改）。 */
const B_RISE_YAW = GR_YAW;
...
/* 爬起來那一段，抬升收到「剩不到這麼多格」就跟躺平角一起歸零（世界單位，v1.202.1）。
   見 hurtBeast 最後那一段。 */
const B_UP_EPS = 0.01;
/* 落地那一刻的躺平角**照舊歸零**——為什麼不「把空中的翻滾角接著用」，量過了，
   見 開發筆記〈落地不能「接著空中的角度躺下去」：兩套算法的樞紐不同〉。
   一句話：飛在半空是**繞身體中段**轉、躺在地上是**繞腳底**轉，兩套只有在角度 0
   才重合（同一個 90° 的姿勢，兩套算出來的原點差巨人 9.74 格、獅鷲 4.02 格）。
   保留角度的話，省下的「姿勢跳」會換成更大的「樞紐跳」——實測反而從 2.31 變 5.33 格。 */
...
function tossBeast(m, vx, vy, vz, lit) {
  ...
  m.roll = 0;
  /* **大的被推得少、轉得慢**（v1.203.0，使用者定案）。原本力道與翻滾角速度完全不看體型，
     結果十五格高的巨人被**一支箭**打中會整隻離地（實測滯空 0.25 秒、飛高 0.20 格、
     水平被推 2 格）——那本身就是件怪事，而且那正是「倒地會亂跳」的來源：
     落地那一刻躺平角要歸零（理由見上面那一段），空中累積的角度越大，那一下跳得越兇。
     折扣照體型算、不另外挑數字：以猴子（DOOM_SC）為基準，比猴子大的才打折，
     比猴子小的維持原樣（min(1, …)）。牛羊的 sc 是 DOOM_SC × 0.90~1.08，
     所以大隻的那幾隻會輕微打折，那正是「照體型」該有的樣子。
     **兩個效果會相乘**：vy 打折 → 滯空跟著變短 → 空中累積的翻滾角再乘一次折扣。
     實測巨人被一支箭打中，落地帶著的角度中位數 125° → 38.9°，
     落地那一幀畫面上那一團跳 2.31 → 0.87 格。 */
  const bk = Math.min(1, DOOM_SC / (m.sc || 1));
  m.vx = vx * bk; m.vy = vy * bk; m.vz = vz * bk;
  m.tsp = rr(4, 9) * bk * (Math.random() < 0.5 ? -1 : 1);   // 翻滾的角速度
  if (lit) m.lit = 1;
  reaim(m);
  return true;
}
