...
/* wind：要不要加那一圈往外掃的風壓（核彈與爆裂魔法專用，見 spawnWind）。
   ...
   隕石 v1.151 放大到 46（比核彈還大）之後**照樣不給**：它走的是 crash 那一路
...
/* ── 隕石 ───────────────────────────────────────────────
   ...
   範圍是投石機石頭的十倍（v1.151 放大 5 倍），威力介於石頭與定時炸彈之間——
   它的重點是火不是威力：
...
/* v1.151（使用者：「隕石大幅提高大小 約5倍(包含隕石本體&破壞範圍)」）：
   石身 2 → 10 格、破壞範圍 9.2 → 46（＝投石機石頭的十倍，比核彈的 30 還大一圈）。
   **威力（MET_POW）沒動**：使用者要的是「大小」不是「打得更痛」，而半徑本身
   就是破壞量的主因——explode 的力道照 1−d/R 衰減，46 這個半徑等於把整片掃進去。
...
const MET_S = 10;               // 石身畫多大（v1.150 之前是 2）；sweepRock 也拿它當碰撞半徑
/* 包住石頭那圈「火頭」（見 stepMeteors）。**撒得開，但不等比放大**：
   散布半徑跟著石身走，不然火全躲在 10 格的石頭裡面；每一顆的大小只放 2.5 倍、
   改用「多兩倍顆數」補上去——等比放大 5 倍實測是一片糊在一起的大黃板（截圖確認過），
   跟那邊「火苗要小、要密、要短命」那條原則正好相反。 */
const MET_FIRE = MET_S / 2;     // 撒開的半徑倍率（×5）
const MET_FIRE_S = MET_S / 4;   // 每一顆的大小倍率（×2.5）
const MET_R = ROCK_R * 10;      // 破壞範圍（46）
...
function stepMeteors(dt) {
  ...
  for (let i = meteors.length - 1; i >= 0; i--) {
    ...
    m.em += dt * 300;                              // v1.151：顆數兩倍（見 MET_FIRE）
    while (m.em >= 1) {
      m.em--;
      ...
      /* v1.151：火頭從三顆一顆改成一半一顆（石頭大 5 倍，要更多顆才包得住），
         散布跟著石身放大、每顆只放 2.5 倍（見 MET_FIRE／MET_FIRE_S）。
         尾巴那些（沿著路徑撒的）維持原樣——它們要小、要密，理由見上面。 */
      ...
      hot.push({
        ...
        rx: Math.random() * 6, ry: Math.random() * 6,
        s: head ? rr(1, 1.8) * MET_FIRE_S : rr(0.24, 0.62),
...
/* ── 火球 ───────────────────────────────────────────────
   ...
   **v1.151 起這兩個數字自己一組，不再寫成 MET_R／MET_POW**：使用者那句話講的是
   v1.146 當時的隕石（9.2／16），而隕石在 v1.151 被指定放大 5 倍（範圍 46）。
   跟著走的話，一隻飛龍會吐出比核彈還大一圈的火球——那不是使用者要的東西。 */
