...
const GATE_CONV = 0.32;
const GATE_SPRAY = 0.30;         // 落點再抖多少（打擊範圍的幾成）
const GATE_AIM_K = 0.45;
...
function aimGate(g, p) {
  ...
  /* 落點一定要**比這個門低**（v1.132.1）：門陣壓低之後有近一半的門低於建築頂，
     ...
     還有 30 把掛在場上）。 */
  const top = Math.min(Math.max(2, (bp ? bp.height : 12) * 0.9), Math.max(0.8, p.y * GATE_AIM_K));
