...
/* 巨人（v1.192）：一聲很慢、很低的悶吼 ＋ 一記腳步的重量。
   **三種聲音要分得開**（場上有機會同時出現）：猴子 220／330、龍吼 88 滑到 48，
   這一聲 64 滑到 40——比龍低一個四度，而且拖得比牠長（1.6 秒 vs 1.15）。
   波形跟龍一樣用鋸齒：60 Hz 上下的基音小喇叭根本推不出來，聽得到的是泛音，
   而鋸齒的泛音是 1/n、三角是 1/n²（同 sndNuke 那一段量過的事）。
   噪音那一支切在 300（龍是 420）：要的是「一大塊東西踏在地上」的悶，不是氣息的嘶。 */
function sndGiant() {
  tone(64, 1.6, 'sawtooth', 0.085, 0.62, 'giant');
  noise(0.7, 0.14, 300);
  setTimeout(() => tone(48, 1.1, 'sawtooth', 0.06, 0.7, 'giant'), 380);
}
...
/* 箭雨齊射那一聲（v1.171）。配方就是 v1.136 從王之財寶身上**拿掉**的那一記——
   2100Hz 往下滑的鋸齒 ＋ 切在 5200 的短噪音。使用者當時說它「像是弓箭聲」
   （量下去 2kHz 以上占 96.7%，見 sndBlade 那段的比較）：對王之財寶是缺點，
   對真的弓箭正好。一輪只放一聲——八十支箭各響一次的話 voiceOK 也只讓三聲過去。
   噪音的低通刻意跟別人不同號（5200）：voiceOK 拿低通頻率當同一支音效的 key，
   撞號會跟那一支互相吃掉名額（理由同 sndGateHit 的 520）。 */
function sndBow() { tone(2100, 0.16, 'sawtooth', 0.05, 0.28, 'bow'); noise(0.12, 0.055, 5200); }
...
/* 格子編號是一個**整數**，不是 'cx:cz' 字串（v1.160.1）。整地那一段每幀要查上萬次
   ...
   （泰姬瑪哈陵 9000 建材，見 開發筆記〈推土機推過去還是會掉幀〉）。
...
/* separate 的溫和版：只算一輪、力道打折、位移還給上限。
   ...
   **這是整地那一段最貴的一支**（v1.151.2，使用者：「9000 塊積木時一排推土機推過去
   ...
   （見 開發筆記〈一排推土機推過去會掉幀（v1.151.2）〉，e2e 有一條守著）。
